...
\ifthenelse{\boolean{lab1} \OR \boolean{lab2} \OR \boolean{lab3} \OR \boolean{all}}{
...
This Developer's Guide has been updated after the first time we submitted the assignment. All the changes made are listed at the end of this guide.
}
...
No major update was done for the final submission, besides renaming a few folders and update documentation. Indeed, the group that evaluated us didn't find any major problems. Furthermore, the next interesting feature would have been to add a user system on our server. However, it would have required a lot of extra programming that we felt wasn't necessary for this course. Indeed, we would have needed to create a HTML/CSS/JS interface for authentication which is a painful process. 
