...
\item Please use the User Guide to see how you can use our plugin.
\item Profit!
\end{enumerate}
...
\section{Laboratory n$\degree$2}
The sequence is identical to the first laboratory.
%\section{Laboratory n$\degree$3}
...
No new limitations were added except on Linux, where you cannot use GTK3. Here are the updated limitations that are inherited from the previous laboratory. 
...
\item Urgent 
\begin{itemize}
\item Add buttons to control the player in the toolbar. A few updates should be made to the player observers (detect that the player is paused for instance). Overall, it shouldn't be too complicated.
\item Add new services, namely Spotify and Deezer. We should have more to add these. It shouldn't take too long.
\item Do the Javadoc.
\end{itemize}
\item Intermediate
\begin{itemize}
